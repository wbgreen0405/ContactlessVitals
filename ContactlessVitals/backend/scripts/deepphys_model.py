...
class DeepPhys(nn.Module):
    def __init__(self, in_channels=3, nb_filters1=32, nb_filters2=64, kernel_size=3, dropout_rate1=0.25,
        ...
        # Motion branch
        self.motion_conv1 = nn.Conv2d(self.in_channels, self.nb_filters1, kernel_size=self.kernel_size, padding=1, bias=True)
        ...
        # Appearance branch
        self.apperance_conv1 = nn.Conv2d(self.in_channels, self.nb_filters1, kernel_size=self.kernel_size, padding=1, bias=True)
        ...
        # Dense layers (input dims vary by img_size)
        if img_size == 36:
    ...
    def forward(self, inputs, params=None):
        # Split input into motion (first 3 channels) and appearance (last 3 channels) branches
        diff_input = inputs[:, :3, :, :]
        raw_input = inputs[:, 3:, :, :]
